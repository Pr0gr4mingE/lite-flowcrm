// src/components/features/modals/forms/form-detalhe-negocio-pj.tsx
"use client";

import { useFormDetalheNegocioPj } from "@/hooks/modals/forms/use-form-detalhe-negociacao-pj.hook";

interface FormDetalheNegocioPjProps {
  negocioId: string | null;
  onCancel: () => void;
  onSuccess: () => void;
}

export function FormDetalheNegocioPj({ negocioId, onCancel, onSuccess }: FormDetalheNegocioPjProps) {
  const {
    carregando, salvando,
    titulo, setTitulo,
    valor, setValor,
    ...
    cnpj, setCnpj,
    handleSalvar
  } = useFormDetalheNegocioPj(negocioId, onSuccess);
    ...
    <form onSubmit={handleSalvar} className="p-6 space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <input type="text" value={cnpj} onChange={(e) => setCnpj(e.target.value)} placeholder="00.000.000/0000-00" className="w-full border rounded-md p-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none" />
        </div>
        <div>
